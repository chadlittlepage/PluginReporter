...
//  GarageBand uses the same package format as Logic Pro
//  This parser delegates to LogicProParser
//

import Foundation

/// Parser for GarageBand project files
class GarageBandParser: DAWParser {
    ...
    static func parseProject(url: URL) throws -> ParsedProject {
        ...
        // GarageBand projects are package directories (same as Logic Pro)
        guard FileManager.default.fileExists(atPath: url.path) else {
            throw ParserError.invalidFileType
        }

        // Look for ProjectData file inside the package (same structure as Logic)
        let projectDataURL = url.appendingPathComponent("Alternatives")
            .appendingPathComponent("000")
            .appendingPathComponent("ProjectData")

        // Fallback to root-level ProjectData
        ...
        if FileManager.default.fileExists(atPath: projectDataURL.path) {
            ...
            throw ParserError.invalidProjectData("ProjectData file not found in GarageBand project package")
        }

        // Read and parse the plist (same format as Logic Pro)
        ...
        // Parse using same logic as Logic Pro parser
        ...
        return ParsedProject(
            name: projectName,
            sourceFile: url,
            dawType: .garageBand,  // Mark as GarageBand, not Logic
            tracks: tracks,
            tempo: tempo,
    ...
    private static func parseTracks(from plist: [String: Any]) -> [ParsedTrack] {
        ...
        // If no tracks found, try deeper search
        if tracks.isEmpty {
    ...
    private static func parsePlugins(from pluginArray: [[String: Any]], trackName: String, trackIndex: Int) -> [ParsedPlugin] {
        ...
        for (deviceIndex, pluginDict) in pluginArray.enumerated() {
            ...
            var format: PluginFormat = .AU  // GarageBand primarily uses AU
            ...
            // Apple plugins (GarageBand has many built-in)
    ...
    private static func deepSearchTracks(in plist: [String: Any]) -> [ParsedTrack] {
        ...
        if !foundPlugins.isEmpty {
            let plugins = parsePlugins(from: foundPlugins, trackName: "GarageBand Project", trackIndex: 0)
    ...
    private static func extractVersion(from plist: [String: Any]) -> String? {
        if let version = plist["Version"] as? String {
            return "GarageBand \(version)"
        }
        if let version = plist["ApplicationVersion"] as? String {
            return "GarageBand \(version)"
        }
        if let version = plist["GarageBandVersion"] as? String {
            return "GarageBand \(version)"
        }
        return nil
    }
