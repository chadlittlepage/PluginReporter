...
struct PluginListView: View {
    ...
    private func computeFilteredAndSorted() {
        let start = Date()
        print("⚙️ [COMPUTE START] computeFilteredAndSorted() with \(plugins.count) plugins")
        var result = plugins
        ...
        print("✅ [COMPUTE DONE] computeFilteredAndSorted() took \(String(format: "%.3f", duration))s")
        computeConsolidated()
    }
    ...
    private func computeConsolidated() {
        let start = Date()
        print("⚙️ [COMPUTE START] computeConsolidated() with \(cachedFilteredSorted.count) plugins")
        ...
        let duration = Date().timeIntervalSince(start)
        print("✅ [COMPUTE DONE] computeConsolidated() took \(String(format: "%.3f", duration))s, result: \(consolidated.count) plugins")
        computeSectioned()
    }
    ...
    var searchBarBackgroundColor: Color {
        ...
        appearance == "space" ? Color(red: 18/255, green: 18/255, blue: 18/255) : appearance == "dark" ? darkDarker : appearance == "light" ? lightDarker : Color(UIColor.systemGray6)
    }
    ...
    var body: some View {
        NavigationStack {
            ...
            .task(id: plugins.count) {
                // OPTIMIZATION: Only compute on first appearance or when plugins actually change
                guard !hasComputedOnce || filteredPluginsForExport.isEmpty else {
                    print("⏭️ [SKIP COMPUTE] Cache is valid, skipping recomputation")
                    return
                }
                print("🔄 [RECOMPUTE] Running full computation")
            ...
            if let plugin = selectedPluginForViewer {
                ...
                if let imageUrlString = imageUrlString,
                   let imageURL = URL(string: imageUrlString) {
                    ...
                    .onAppear {
                        print("📸 [iPad VIEWER] Plugin: \(plugin.name)")
                        print("📸 [iPad VIEWER] screenshotUrl: \(plugin.screenshotUrl ?? "nil")")
                        print("📸 [iPad VIEWER] thumbnailUrl: \(plugin.thumbnailUrl ?? "nil")")
                        print("📸 [iPad VIEWER] Valid URL found: \(imageURL.absoluteString)")
                    }
                } else {
                    ...
                    .onAppear {
                        ...
                        print("❌ [iPad VIEWER] screenshotUrl: \(plugin.screenshotUrl ?? "nil")")
                        print("❌ [iPad VIEWER] thumbnailUrl: \(plugin.thumbnailUrl ?? "nil")")
                        print("❌ [iPad VIEWER] No valid URL found!")
                    }
                }
            } else {
                VStack(spacing: 12) {
